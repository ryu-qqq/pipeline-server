...
#### Worker -> Outbound 직접 의존에 대한 설계 판단

Inbound Worker(`pipeline_task.py`, `outbox_poller_task.py`)는 Outbound 구현체를 직접 import하고 있다.
이는 Celery Worker가 FastAPI의 `Depends()` 체인을 사용할 수 없기 때문이다.

REST 경로에서는 `dependencies.py`가 구현체를 조립하여 Application 서비스에 주입하지만,
Worker는 요청 컨텍스트가 없으므로 `_build_pipeline_service()` 팩토리 함수에서 직접 조립한다.

구조적으로는 `inbound -> outbound` 직접 참조이나, 다음 이유로 허용 가능한 설계 타협으로 판단했다.

- 비즈니스 로직은 Application 서비스(`PipelineService`, `OutboxRelayService`)에 완전히 위임되어 있다
- Worker 파일은 `dependencies.py`와 동일한 역할(DI 조립)만 수행한다
- Celery 환경에서 FastAPI Depends를 대체할 수 있는 표준적인 방법이 없다

향후 Worker 전용 DI 모듈(`worker_dependencies.py`)을 분리하면 inbound/outbound 간 직접 참조를 제거할 수 있다.
...
graph LR
    subgraph MONGO ["MongoDB -- 유연한 쓰기"]
        M1["raw_data<br/>원본 JSON/CSV"]
        M2["analyze_tasks<br/>작업 상태+진행률"]
        M3["outbox_messages<br/>이벤트 발행 보장"]
    end
...
```mermaid
graph TD
    FILES["원본 파일<br/>selections.json / odds.csv / labels.csv"]
...
graph LR
    ...
    subgraph CEL ["Celery Process"]
        BEAT["Beat (스케줄러)"]
        WORK["Worker"]
        T1["pipeline.process_analysis<br/>max_retries=1"]
        T2["outbox.relay<br/>5초 간격"]
        T3["outbox.recover_zombies<br/>60초 간격"]
    end
