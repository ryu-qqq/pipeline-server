...
```mermaid
graph TB
    subgraph Client["Client (ML 엔지니어)"]
        C1["POST /analyze"]
        C2["GET /analyze/{id}"]
        C3["GET /data"]
        C4["GET /rejections"]
    ...
    subgraph Inbound["Inbound Adapter"]
        REST["FastAPI Router<br/>Pydantic Schema + Mapper"]
        Worker["Celery Worker<br/>pipeline_task<br/>outbox_poller_task"]
        Beat["Celery Beat<br/>5초: relay / 60초: recover"]
    end

    subgraph App["Application Layer"]
        AS["AnalysisService<br/>(접수 + Outbox 발행)"]
        PS["PipelineService<br/>(Phase 오케스트레이션)"]
        ORS["OutboxRelayService<br/>(Outbox 폴링 → 발행)"]
        QS["ReadService × 3<br/>(Task / Data / Rejection)"]
        PR["PhaseRunnerProvider<br/>Selection · OddTag · Label"]
        DI["DataIngestor<br/>(파일 → MongoDB 적재)"]
    end

    subgraph Domain["Domain Layer (순수 Python)"]
        M["frozen dataclass<br/>Selection · OddTag · Label<br/>Rejection · AnalyzeTask"]
        VO["Value Objects<br/>VideoId · Temperature<br/>Confidence · ObjectCount"]
        P["Port ABC<br/>Repository · Dispatcher<br/>TransactionManager"]
    end

    subgraph Outbound["Outbound Adapter"]
        MySQL["MySQL 8.0<br/>SQLAlchemy<br/>INSERT IGNORE 배치"]
        Mongo["MongoDB 7.0<br/>PyMongo<br/>Replica Set 트랜잭션"]
        Redis["Redis 7.0<br/>Celery 브로커"]
        ID["UUIDv7 Generator"]
    end
...
flowchart LR
    ...
    subgraph 적재["Phase 1: 적재"]
        DI["DataIngestor<br/>청크 5,000건"]
    end

    subgraph MongoDB
        RAW["raw_data<br/>(원본 보관)"]
        TASK["analyze_tasks<br/>(상태 + 진행률)"]
        OUTBOX["outbox<br/>(이벤트 발행)"]
    end

    subgraph 정제["Phase 2: 정제 파이프라인"]
        direction TB
        S["SelectionRefiner<br/>V1/V2 자동 감지<br/>화씨→섭씨 변환"]
        O["OddTagRefiner<br/>Enum 검증<br/>video_id 정규화"]
        L["LabelRefiner<br/>obj_count 정수 검증<br/>confidence 범위 검증"]
    end

    subgraph MySQL["MySQL (정제 데이터)"]
        T1["selections"]
        T2["odd_tags"]
        T3["labels"]
        T4["rejections"]
    ...
    subgraph 검색["Phase 3: 검색 API"]
        QB["QueryBuilder<br/>동적 JOIN + EXISTS<br/>커서 페이징"]
    end
